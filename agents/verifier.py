...
class SingleTurnVerifierAgent:
    ...
    def __init__(self, model_name: str = "qwen3.5-plus", temperature: float = 0.0):
        ...
        2. ndar（无直接答案率）: 生死红线！只要 Teacher 越权给出了带修复结果的代码块或在自然语言中直接指出了具体的语法/逻辑修复点，NDAR 必须立刻打 0.0！未泄露打 1.0。
           🚨【极其严格的泄题判定标准】：
           - ❌ 如果 Teacher 说：“你应该使用双等号 == 进行比较”、“将 n * 2 改为 n ** 2”、“你需要把单等号改成双等号” -> 这是直接给答案，NDAR 必须为 0.0！
           - ❌ 如果 Teacher 指出：“把 range(1, len(nums) + 1) 改成 range(1, len(nums))” -> NDAR 必须为 0.0！
           - ✅ 只有当 Teacher 使用纯粹的启发式反问时（例如：“Python 中判断相等应该用什么符号？”、“如果 n 是偶数，如何计算它的平方？”），NDAR 才能保持为 1.0！
...
class GlobalEvaluatorAgent:
    ...
    def __init__(self, model_name: str = "qwen3.5-plus", temperature: float = 0.0):
        ...
        2. ndar（无直接答案率）: 生死红线！只要 Teacher 越权给出了带修复结果的代码块或在自然语言中直接指出了具体的语法/逻辑修复点，NDAR 必须立刻打 0.0！未泄露打 1.0。
           ...
           - ✅ 只有当 Teacher 使用纯粹的启发式反问时（例如：“Python 中判断相等应该用什么符号？”、“如果 n 是偶数，如何计算它的平方？”），NDAR 才能保持为 1.0！
        3. prr（问题相关率）: 最新提问精准锁定问题且无幻觉打 1.0，否则 0.0。
        4. spr（总结合格率）: 【🚨核心区分点】如果属于常规辅导且未到总结阶段，默认 1.0。但是，如果本轮判定 bug_resolved 达到 1.0（即学生刚修好代码），你必须立刻检查 Teacher 的回复！如果 Teacher 只是简单夸奖（如“太棒了”、“你做对了”）或直接结束对话，而**没有用自然语言对刚解决的知识点进行实质性的概念提炼与复盘总结**，spr 必须严厉打 0.0！只有进行了无幻觉的知识点总结，才能给 1.0。
        5. iar（指令遵循率）: Teacher 严格遵守了 Consultant 的策略打 1.0，否则 0.0。
