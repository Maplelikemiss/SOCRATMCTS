...
class VerifierAgent:
    ...
    def __init__(self, model_name: str = "qwen3.5-plus", temperature: float = 0.0):
        ...
        2. 单轮即时评估 (NDAR, PRR, SPR, IAR)：
           ...
           - NDAR（无直接答案率）是生死红线！只要 Teacher 越权给出了带修复结果的代码块或直接指出了修复点，NDAR 必须为 0.0。- 【豁免条款】：如果学生在当前或之前的轮次中【已经】给出了正确的修复代码，那么老师在后续回复中复盘、总结或解释原理是合法的教育行为，此时 NDAR 必须保持为 1.0！只有当学生还没修好 Bug，老师就提前越权把答案说出来了，NDAR 才是 0.0。
